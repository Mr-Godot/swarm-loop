...
5. Create the run directory `swarm-runs/<YY-MMDD-slug>/` (local root: `C:\Users\godot\_agents\x-temp\swarm-runs\`; on studio: `~/swarm-runs/`). Copy `templates/render_progress.py` into it. Write `GOAL.md` from `templates/GOAL.md`. It is immutable from this point.
6. Decide where it runs. More than about 6 parallel agents or more than an hour of expected work: dispatch to studio with the `lx-dispatch` skill (LX is retired; the skill now targets studio). Otherwise run locally.
